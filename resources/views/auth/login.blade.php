    ...
    <div x-data="loginForm(@js($mistakes))">
        <x-card>
            <div class="mb-6">
                ...
                <p class="mt-1 text-sm text-slate-500">受講生マイページにアクセスします。</p>
            </div>
            ...
            <form method="POST" action="{{ route('login.post') }}" @submit="onSubmit">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="email" name="email" inputmode="email" autocomplete="username"
                               ...
                               class="input pl-10" placeholder="student@example.com" required>
                    </div>
                ...
                <button type="submit" class="btn-primary w-full justify-center" :disabled="remaining <= 0">
                    ...
                    <span x-text="remaining > 0 ? 'ログイン' : '試行上限に達しました'"></span>
                </button>
            </form>

            {{-- デモアカウント --}}
            <div class="mt-6 rounded-xl border border-dashed border-brand-200 bg-brand-50/50 p-4">
                <div class="flex items-center justify-between">
                    <div class="text-xs font-semibold text-brand-700">デモアカウント</div>
                ...
                <dl class="mt-2 space-y-1 text-xs text-slate-600">
                    <div class="flex justify-between"><dt>メール</dt><dd class="font-mono">student@example.com</dd></div>
        ...
        <div class="mt-6 space-y-3 text-center text-sm">
            <p class="text-slate-500">
                アカウントをお持ちでない方は
                <a href="{{ route('register') }}" class="font-semibold text-brand-600 hover:text-brand-700">新規登録</a>
            </p>
            <div class="flex items-center justify-center gap-4 text-slate-400">
                ...
                <a href="{{ route('teacher.login') }}" class="inline-flex items-center gap-1 hover:text-slate-600">
                    <x-icon name="academic" class="h-4 w-4" /> 講師ログイン
                </a>
            </div>
        </div>
    </div>
    ...
    <script>
        function loginForm(mistakes) {
            return {
                ...
                autofill() {
                    this.$refs.email.value = 'student@example.com';
