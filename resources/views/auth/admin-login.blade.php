    ...
    <div x-data="loginForm(@js($mistakes))">
        <x-card>
            <div class="mb-6">
                ...
                <h1 class="text-2xl font-bold text-slate-900">管理コンソール</h1>
                <p class="mt-1 text-sm text-slate-500">運用管理者向けのログインページです。</p>
            </div>
            ...
            <form method="POST" action="{{ route('admin.login.post') }}" @submit="onSubmit">
                @csrf

                <div class="mb-4">
                    <label for="email" class="label">管理者メールアドレス</label>
                    <div class="relative">
                        ...
                        <input type="text" id="email" name="email" inputmode="email" autocomplete="username"
                               ...
                               class="input pl-10" placeholder="admin@example.com" required>
                    </div>
                ...
                <button type="submit" class="btn-primary w-full justify-center" :disabled="remaining <= 0">
                    ...
                    <span x-text="remaining > 0 ? '管理コンソールにログイン' : '試行上限に達しました'"></span>
                </button>
            </form>

            <div class="mt-6 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4">
                <div class="flex items-center justify-between">
                    <div class="text-xs font-semibold text-slate-700">デモアカウント</div>
                ...
                <dl class="mt-2 space-y-1 text-xs text-slate-600">
                    <div class="flex justify-between"><dt>メール</dt><dd class="font-mono">admin@example.com</dd></div>
